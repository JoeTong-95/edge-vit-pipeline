...
Draw bounding boxes, track IDs, and labels onto an input-layer stream.
...
def draw_tracks(frame, tracking_pkg):
    for trk in tracking_package_rows(tracking_pkg):
        ...
        label = f"ID={trk['tracking_layer_track_id']} {trk['tracking_layer_detector_class']} {trk['tracking_layer_confidence']:.2f}"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Visualize tracking on input-layer output")
    ...
    try:
        while True:
            ...
            average_infer_fps = processed_frames / total_yolo_seconds if total_yolo_seconds > 0 else 0.0
            ...
            if frame_id % 50 == 0:
                ...
                print(f"Frame {frame_id}/{total_frames_text} - {len(track_rows)} tracks - {fps_actual:.1f} FPS - inferFPS {average_infer_fps:.1f} - yolo {metrics.get('yolo_ms', 0.0):.1f} ms - track {metrics.get('track_ms', 0.0):.1f} ms - loop {metrics.get('loop_ms', 0.0):.1f} ms")
