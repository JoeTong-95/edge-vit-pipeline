...
CONFIG_LAYER_DIR = Path(__file__).resolve().parent.parent
if str(CONFIG_LAYER_DIR) not in sys.path:
    sys.path.insert(0, str(CONFIG_LAYER_DIR))
...
class ConfigurationLayerTests(unittest.TestCase):
    def test_load_config_applies_defaults(self) -> None:
        config = load_config({"config_input_path": "data/sample3.mp4"})
